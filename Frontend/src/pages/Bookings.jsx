...
const Bookings = () => {
    ...
    {
        await fetch(`http://localhost:3000/api/cars/`).then((res) => res.json()).then((data) => {
            console.log("Data", data);
            setGetBookedCars(data);    
            console.log("Get Booked Cars", data.find(item => item._id == id));
        });
    }
                            ...
                            <div className='booking-item'>
                                <table className="vertical-table">
                                    <tbody>
                                        ...
                                        <tr>
                                            <th>Car Booked</th>
                                            <td><Link to={`/cars/${bookingInfo.carId}`}>
                                                {getBookedCars && getBookedCars.find(item => item._id == bookingInfo.carId).carName}
                                                </Link>
                                            </td>
                                        </tr>
                                        <tr>
                                        ...
                                        <tr>
                                            <th>Journey Date</th>
                                            <td>{formatDate(bookingInfo.journeyDate)}</td>
                                        </tr>
                                        <tr>
                                            <th>Journey Time</th>
                                        ...
                                        <tr>
                                            <th>Booked Date</th>
                                            <td>{formatDate(bookingInfo.bookedDate)}</td>
                                        </tr>
                                        <tr>
                                            <th>Booked Time</th>
                                            <td>{bookingInfo.bookedTime}</td>
                                        </tr>
                                        {checkExpired(bookingInfo.expiryTime) !== "Expired" ? <tr>
                                            <th>Expiry Date</th>
                                            <td>{checkExpired(bookingInfo.expiryTime)}</td>
                                        </tr> : 
